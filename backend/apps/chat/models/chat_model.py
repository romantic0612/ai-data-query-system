...
ANALYSIS_MODE_PROMPTS = {
    "raw": (
        "当前问数模式：原样查询。优先快速、准确返回数据库中的原始明细或直接汇总结果；"
        "不要做无依据的扩展分析；除非用户明确要求图表，否则优先使用表格。"
    ),
    "correlation": (
        "当前问数模式：关联分析。优先从时间、组织、群体、空间、类别等维度做对比，"
        "可以使用多表关联；输出应突出差异、排名、环比/同比、异常点和可能原因。"
    ),
    "prediction": (
        "当前问数模式：数据预测。优先选择时间序列或高频交互数据，先查询足够的历史数据，"
        "再基于趋势、周期、峰值和异常变化给出预测。若数据不足，应明确说明不能可靠预测。"
    ),
    "report": (
        "当前问数模式：报告生成。查询结果应服务于固定报告结构，包括核心结论、关键指标、"
        "维度对比、趋势变化、异常发现、原因分析和建议措施。"
    ),
    "data_demand": (
        "当前问数模式：数据需求文档。围绕用户问题梳理所需数据源、指标口径、计算规则、"
        "应用场景、已有数据、缺失数据和建议补采单位；如无法直接查询，应输出数据建设建议。"
    ),
    "api_query": (
        "当前问数模式：API问数。优先通过已配置的业务系统API获取数据，不要求业务单位推送库表；"
        "关注接口名称、入参口径、返回字段和调用审计，适合权限敏感或实时业务数据。"
    ),
}
...
class AiModelQuestion(BaseModel):
    ...
    def analysis_mode_prompt(self) -> str:
        return ANALYSIS_MODE_PROMPTS.get(self.analysis_mode, ANALYSIS_MODE_PROMPTS["raw"])
    ...
    def sql_user_question(self, current_time: str, change_title: bool):
        ...
        mode_rule = self.analysis_mode_prompt()
        rule = (self.rule + "\n" if self.rule else "") + mode_rule
        return get_sql_template()['user'].format(lang=self.lang, engine=self.engine, schema=self.db_schema,
                                                  question=_question,
                                                  rule=rule, current_time=current_time, error_msg=self.error_msg,
    ...
    def chart_user_question(self, chart_type: Optional[str] = '', schema: Optional[str] = ''):
        mode_rule = self.analysis_mode_prompt()
        rule = (self.rule + "\n" if self.rule else "") + mode_rule
        return get_chart_template()['user'].format(lang=self.lang, sql=self.sql, question=self.question, rule=rule,
...
class ChatQuestionBase(BaseModel):
    ...
    analysis_mode: Optional[str] = Body(description='问数模式', default='raw')
